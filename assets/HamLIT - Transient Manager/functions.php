...
<?php
	...
	*/
	public function suspend_transients_button( $wp_admin_bar ) {
		...
		$action = get_option( 'hamlit_tm_suspend' ) ? 'unsuspend_transients' : 'suspend_transients';
		$label  = get_option( 'hamlit_tm_suspend' ) ? '<span style="color: red;">' . __( 'Unsuspend Transients', 'transients-manager' ) . '</span>' : __( 'Suspend Transients', 'transients-manager' );
	...
	*/
	private function get_total_transients( $search = '' ) {
		...
		if( ! empty( $search ) ) {
			...
			$count = wp_cache_get( 'hamlit_transients_count_' . sanitize_key( $search ) );
			...
			if( false === $count ) {
				...
				wp_cache_set( 'hamlit_transients_' . sanitize_key( $search ), $count, '', 3600 );
			}

		} else {

			$count = wp_cache_get( 'hamlit_transients_count' );
			...
			if( false === $count ) {
				...
				wp_cache_set( 'hamlit_transients_count', $count, '', 3600 );
			}
	...
	*/
	public function maybe_block_update_transient( $value, $option, $old_value ) {
		...
		if( ! get_option( 'hamlit_tm_suspend' ) ) {
			return $value;
		}
...
$tm = new hamlit_Transients_Manager;
unset( $tm );