<?php

//All Options
$options = [
    'hamlit_addextraprofilefields',
    'hamlit_enablelivechat',
        'hamlit_loginchat',
        'hamlit_hidechat',
    'hamlit_changemyaccountsections',
    'hamlit_changewoocolumns',
    'hamlit_customloginpage', //$clp_options
        'hamlit_custom_login_button',
        'hamlit_custom_login_button_hover',
        'hamlit_login_logo',
        'hamlit_login_logo_height',
    'hamlit_deferjs',
    'hamlit_disableusers',
    'hamlit_displayuserid',
    'hamlit_hidetitle',
    'hamlit_livechat',
        'hamlit_livechatcode',
        'hamlit_hidelivechat',
    'hamlit_onlysellto',
    'hamlit_removedashwidgets',
    'hamlit_removepostcount',
    'hamlit_removeproductcount',
    'hamlit_removeprofilefields',
    'hamlit_removenags',
    'hamlit_removewpbranding', // $rwpb_options
        'hamlit_howdy_text',
        'hamlit_eliminate_logo',
        'hamlit_eliminate_new_menu',
        'hamlit_eliminate_help',
        'hamlit_removescreenoptions',
        'hamlit_lower_left_text',
        'hamlit_hide_wp_version',
        'hamlit_remove_comp_dashboards',
        'hamlit_left_widget_title',
        'hamlit_left_widget_content',
        'hamlit_right_widget_title',
        'hamlit_right_widget_content',
        'hamlit_init_removewpbranding',
    'hamlit_renamesidebaritems',
    'hamlit_sitehealth',
    'hamlit_bloginfosc',
    'hamlit_stopsmoothnessjquery',
    'hamlit_transientmanager',
    'hamlit_viewpageidentifier'
];

//Simple Options
$simple_options = [ 
    'hamlit_addextraprofilefields',
    'hamlit_enablelivechat',
        'hamlit_loginchat',
        'hamlit_hidechat',
    'hamlit_changemyaccountsections',
    'hamlit_changewoocolumns',
    'hamlit_customloginpage', //$clp_options
    'hamlit_deferjs',
    'hamlit_disableusers',
    'hamlit_displayuserid',
    'hamlit_hidetitle',
    'hamlit_livechat',
        'hamlit_livechatcode',
        'hamlit_hidelivechat',
    'hamlit_onlysellto',
    'hamlit_removedashwidgets',
    'hamlit_removepostcount',
    'hamlit_removeproductcount',
    'hamlit_removeprofilefields',
    'hamlit_removenags',
    'hamlit_removewpbranding', // $rwpb_options
    'hamlit_renamesidebaritems',
    'hamlit_sitehealth',
    'hamlit_bloginfosc',
    'hamlit_stopsmoothnessjquery',
    'hamlit_transientmanager',
    'hamlit_viewpageidentifier'
];

//Remove WP Branding Options
$rwpb_options = [
        'hamlit_howdy_text',
        'hamlit_eliminate_logo',
        'hamlit_eliminate_new_menu',
        'hamlit_eliminate_help',
        'hamlit_removescreenoptions',
        'hamlit_lower_left_text',
        'hamlit_hide_wp_version',
        'hamlit_remove_comp_dashboards',
        'hamlit_left_widget_title',
        'hamlit_left_widget_content',
        'hamlit_right_widget_title',
        'hamlit_right_widget_content',
        'hamlit_init_removewpbranding',
];

//Custom Login Options
$clp_options = [
        'hamlit_custom_login_button',
        'hamlit_custom_login_button_hover',
        'hamlit_login_logo',
        'hamlit_login_logo_height',
];

?>